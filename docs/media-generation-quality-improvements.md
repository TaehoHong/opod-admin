...
- 운영 화면에 표시되는 프롬프트, 레퍼런스와 실제 provider 요청이 일치해야
  한다.
...
- 기획에 사용한 유효 레퍼런스와 실제 실행 레퍼런스를 일치시키고, 빌드 대상
  모델과 실행 provider 모델이 달라지면 제출 전에 영구 실패시킨다.
...
5. **완료** — 샷별 실제 생성 모델과 프롬프트 대상 모델 일치 검증
...
3. 운영 화면에 실제 선택·해결 레퍼런스와 라우트 표시
...
6. 빌더 대상 모델과 실제 provider 모델 불일치 차단
...
- 샷별 target model과 provider 라우트 일치
- 레퍼런스 편집 시 설명 보존
