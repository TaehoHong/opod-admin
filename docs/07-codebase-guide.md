...
| Evaluations    | `src/admin/evaluations/`, `src/worker/evaluation*`, `prompts/*evaluator.ts`                                        | 기획·프롬프트·생성 이미지 비차단 평가, 후보별 하드 실패, draft별 조회, 수동 집계 리포트 | `EvaluationWorkerService`, `EvaluationsService` | evaluator/worker/evaluations specs, draft RTL test |
