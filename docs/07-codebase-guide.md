...
| V3 post pipeline          | `PostPipelineV3Runner` + `DraftWorkerRepository`         | 신규 draft만 version pin; PostPlan → ImagePlan → PromptSet, artifact revision/hash/CAS, legacy V2 불변                                                                                                                                                | V3 runner/agent specs, generation E2E                |
| V3 model policy           | `src/worker/image-model-policy.ts`                       | exact model ID의 capability와 reference slot/order만 소유; scene 의미나 generation parameter를 만들지 않음                                                                                                                                             | prompt Agent, generation worker                      |
