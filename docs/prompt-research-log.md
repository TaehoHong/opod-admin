...
- **관측 2 (2026-08-16, V4 draft `01a0089b…`, 톤 힌트 없는 입력 조건)** — R1·R2
  또 미발생(2/2). 폰은 반사 안 손 위, preserve는 구체 요소만. 반복 관측: (a) 기획
  "15도" → 렌더는 훨씬 더 틀어짐(2/2), (b) **R3 재발 3회째** — 마루 위 운동화.
  기획은 어디 서는지 정하지 않았고 평가자는 소유 차원이 없어 못 잡는다(§19.1
  #13). R3는 이 프롬프트의 범위 밖으로 남겨둔 결정이므로 기각 근거는 아니지만,
  게시 가능 판정에서는 매번 걸릴 항목이다.
...
- **관측 3 (2026-08-16, 한소이 V4 draft `01a0093b…`, 두 번째 캐릭터 · 첫 비(非)거울
  컷)** — 새 결함 유형: **혼자인 캐릭터를 누가 뒤에서 찍은 구도.**
  - 기획: "양손이 필름 네거티브를 펼친다(visibleParts: 양손·팔뚝, 얼굴 없음)" +
    captureSetup "손으로 든 카메라를 책상 55cm 위에서 아래로, 촬영자는 책상 앞쪽
    가장자리". 양손이 다 화면에 있는데 카메라를 손으로 든다 — 한 사람으로는
    성립하지 않는다. v3의 "한 사람으로 동시에 가능해야 한다" 문장이 있는데도
    났다. 렌더는 오른쪽 어깨 뒤에서 내려다본 3인칭(등·머리·팔 노출).
  - **원인은 규칙 부재가 아니라 입력 누락.** 한소이 `content_style` 블록에 이 컷을
    정확히 금지하는 문장이 있다 — "혼자면 고정면 + 셀프타이머, 기본은 소이 시점의
    손 디테일, **손에 든 셀카나 누군가 뒤에서 따라 찍는 구도는 쓰지 않는다**". 그런데
    ③ 입력 조립(`post-pipeline-v3.runner.ts` `runImagePlanning`)이
    `relevantContext`에서 `content_style`을 **제외**한다(글쓰기 프로필로 분류돼
    ②·⑥에만 감). 플래너가 본 것은 `preferences`의 "셀카 싫어함 — 뒷모습이나
    손만"뿐이라 손 컷을 고른 것까지는 옳았고, 촬영 방식은 알 길이 없었다.
  - 가짜 초록불 2건: 기획 평가 `capture_plausibility` 5점(누가 카메라를 드는지 안
    본다), 생성 이미지 평가 `character_presentation` 5·`capture_and_composition`
    5(계획은 손·팔뚝만인데 등·머리·어깨가 나온 것을 못 잡음). 이미지 평가가 잡은
    것은 "인화지 더미가 셋이 아니라 둘"(major)뿐.
  - **어블레이션 (같은 환경 레퍼런스, nano-banana-pro/edit, 시드 11·22·33):**

    | 조건 | 바꾼 것 | 1인칭(뒷모습 없음) |
    |---|---|---|
    | 원본 (파이프라인 산출) | — | 0/1 |
    | A | 원본 프롬프트 그대로 | 0/3 |
    | B | "with the photographer and device fully outside the frame" 절만 삭제 (④ 평가가 meta_leak minor로 잡은 단어) | 0/3 |
    | C | 촬영 문장을 "본인 시점, 몸·머리·어깨는 보이지 않음"으로 교체 | 0/3 |
    | D1 | **장면 계약**을 바꿈 — 한 손만 아래에서 들어와 네거티브를 든다, 폰을 다른 손에 들고 직접 내려찍음, 데스크만 타이트하게 | **3/3** |
    | D2 | D1 + 워커가 붙이는 장소 네거티브 꼬리 제거 | **3/3** |
    | E | 원본 프롬프트, **환경 레퍼런스 없이** t2i(`nano-banana-pro`) | 1/3 (얼굴도 2/3에서 노출) |

    문구(B·C)로는 0/6, 장면 계약(D)으로는 6/6. **고칠 곳은 ④ 프롬프트가 아니라
    ③의 장면 계약이다** — "양손 + 손으로 든 카메라"라는 장면을 받은 모델은 어떤
    시점 문구를 줘도 뒤에서 찍은 사람을 만든다. D1=D2이므로 장소 네거티브 꼬리는
    이 축의 원인이 아니다. E는 방 모서리 와이드뷰 레퍼런스가 3인칭 쪽으로 미는
    것을 시사하지만(0/9 → 1/3) 주원인은 아니다 — 지렛대 순서는 **장면 계약 >
    레퍼런스 시점 > 촬영 문구**. 종합은 아키텍처 §19.5.
  - 부수 발견 (관측만): 실제 fal 전송본 꼬리에 장소(연남동 원룸)의 네거티브
    `people, body parts, …`가 "Do not include:"로 붙어 나간다 — 손이 나와야 하는
    컷과 모순. 서린 현관 장소도 `people, faces, human reflection, camera
    reflection, shoes, clothes, bags`가 거울 셀피 전부에 붙어 있었다. 이 블록은
    `negative-block-ablation`에서 **유지 결정**(장면 정합성 축에서 일함, `shoes`)이
    난 것이라 제거를 제안하지 않는다. 기록하는 이유는 하나 — **④ 평가자가 보는
    프롬프트와 실제 전송본이 다르다.** 모순 항목이 어느 쪽으로 풀리는지는 아무
    평가자도 안 본다(#12 계약 모순과 같은 계열).
  - 판정: `image-planner-v3` 문장 변경 없음(규칙 추가 안 함). 처방 후보는 코드
    한 줄 — ③ 입력에 `content_style`을 넣고 같은 조건으로 한 판 더 관측. 그래도
    나면 그때 프롬프트를 본다. 이 초안은 ③ 장면이 문제라 ⑤ 컷 재생성으로는 안
    고쳐진다(0/10) — 새 초안으로 다시 돌린다.
...
### 관측 1 (2026-08-16, draft `01a0089b…`, 첫 V4 완주 ①→⑥, ⑦ 게시 대기)

- 기계: ② `post-plan-v2`(캡션 없음) → ③ `image-planner-v3` → ④ 373단어 → ⑤ 1장
  자동 선택 → ⑥ `caption-writer-v1`이 이미지를 실제로 받았다(LLM 로그에
  `image_url` 블록, 4,862 prompt tokens, 3.6초) → 컬럼 기록 → `publish pending`.
  기억 후보 0건.
- 출력: `필라테스 끝나고 집에서 다음 촬영 옷 슬쩍 체크,, 잠 챙겼더니 오늘은
  아이보리 핏이 은근 잘 받네 🤍` / #필라테스 #운동복 #핏체크 #애슬레저 / ko.
- 근거 규칙 준수: 언급 요소 전부 intent 또는 이미지에 있음. 이미지에만 있는 것
  (뒤로 간 벤치, 마루 위 운동화)과 계획에만 있는 것은 언급하지 않았다 — 의도한
  경계가 첫 판에서 지켜졌다(n=1).
- voice 서명 표현(',,' '슬쩍' '오늘' '은근' 🤍) 5개가 한 문장에 몰렸다 — 예시
  캡션의 밀도(1~2개)보다 높다. 과밀 여부는 사람 판정.
- 자연스러움 판정 후보(판정은 운영자): "핏이 잘 받네", "잠 챙겼더니".
- 부수: ②의 premise에 "붓기가 조금 정리된 느낌"이 있다 — v1 캡션에서 지적된
  "자세가 정리된 느낌"과 같은 "X가 정리되다" 패턴이 **상류(②)로 옮겨 갔다.**
  캡션 Agent는 이 표현을 받아 쓰지 않았다(원인 미상, n=1).
- 부수: 이 캐릭터의 초안 4건이 연속 필라테스 거울 셀피인데 `continuity_and_
  novelty` 5점 — recentPosts가 0건(게시된 글이 없음)이라 평가자가 반복을 볼 수
  없다. 반복 억제는 **게시된 글만** 신호가 된다.
- 판정: 보류(n=1). 편집 여부·이유는 운영자 저장 시 `DRAFT_CAPTION_EDITED`로.
- **운영자 정정 4건 (사후, 같은 날):** (1) 필라테스를 센터에서 했으면 보통 그
  자리에서 찍는다 — "집에서"가 어색 (씨앗은 ② premise: 장소 카탈로그가 현관 하나뿐
  + v2의 "충분히 구체적으로" 문장이 사연을 지어냄), (2) "다음 촬영 옷 체크"는 안
  하는 행동 (② premise 발명), (3) "잠 챙겼다"는 안 쓰는 말 — 뜻을 모르겠음 (⑥ 연어),
  (4) '슬쩍'과 '체크'는 같이 안 쓴다 (⑥ 연어). 운영자 방침: **규칙을 더해서 막지
  않는다**("1번처럼 하면 또 제약이 걸린다"). 처방은 빼기(post-planner-v3 = 그
  문장 삭제)와 두 번째 캐릭터 관측.

### 관측 2 (2026-08-16, 한소이 draft `01a0093b…`, 두 번째 캐릭터, ①→⑥ 완주)

- 입력 조건: 서린과 달리 recentPosts 3건 있음(사람이 심은 2건 `📷`, `커피 마시러
  나왔다가 / 햇빛만 잔뜩 담아간 날` + **이전 파이프라인이 생성해 게시한 1건**
  `정동진에서 돌아온 뒤에도, 바다는 카메라 안에서 아직 천천히 흔들리는 중이에요 /
  기다리는 시간까지 한 롤의 일부라고 믿어보기`). memories 11건, premise는
  메모리("정동진 세 롤, 현상 맡김")에 정확히 근거함 — 이번엔 ② 발명 없음.
- 출력: `새벽의 간이역과 아침 바다를 꺼내놓으니, 정동진의 여름이 다시 책상 위에
  앉았다 / 세 롤에 담긴 첫 기록` / #정동진 #필름사진 #코닥골드200 #여름기록.
- 근거 규칙: 언급 요소(간이역 인화지·바다 인화지·책상·네거티브)는 계획과 이미지
  양쪽에 있음 — 준수(2/2). 규칙 준수 항목(1~3문장, 마침표 아끼기, 이모지 절제,
  해시태그 3~5개 지역+필름+계절)도 전부 통과. **그런데 운영자 판정 "자연스럽지
  않다."** 규칙을 다 지켜도 부자연스러운 사례 — §19.2 "사례-원칙"의 캡션판.
- 부자연스러움의 분해 (기록자 판독, 판정은 운영자):
  1. **premise 문장이 캡션으로 샌다 (n=2, 서린·한소이 둘 다).** premise "…첫
     기록을 **꺼내어** 공유한다" → 캡션 "**꺼내놓으니** … **첫 기록**". ②가 사실
     기록용으로 쓴 문장을 ⑥이 표현 재료로 읽는다. 서린에서는 발명된 사연이
     샜고(정정 1·2), 한소이에서는 사실은 맞지만 **문어체 어휘**가 샜다.
  2. **"시적"의 해석이 예시와 다르다.** voice 블록은 "짧은 시적 문장"이라 쓰고
     예시로 `여름이 오래 머물던 날, 강릉`을 준다 — 명사형 종결, 지명 하나, 비유
     없음. 사람이 심은 recentPost도 같은 꼴(`…담아간 날`). 생성본은 의인화
     은유("여름이 책상 위에 앉았다") + 서술형 종결("-았다") — 형용사 "시적"을
     따르고 예시를 안 따랐다.
  3. **명사 과밀.** 두 줄에 새벽 간이역·아침 바다·정동진·여름·책상·세 롤·첫 기록
     = 7개. 예시 캡션은 2~3개. premise를 압축한 문장이지 사람이 남긴 한마디가
     아니다.
  4. `세 롤에 담긴 첫 기록`은 소제목 꼴 — 말이 아니라 라벨.
  5. **되먹임 위험:** recentPosts에 든 이전 생성 캡션(위 "흔들리는 중이에요")이
     같은 과장 은유 계열이다. 부자연스러운 캡션이 게시되면 다음 캡션의 "최근
     습관" 근거가 된다 — 프롬프트는 recentPosts를 "반복 회피 목록"으로만 쓰라고
     하지만 레지스터까지 배우는 것을 막지는 못한다.
- 서린 정정과의 대응: 서린 정정 3·4(연어)와 같은 층이 아니다. 한소이 캡션에는
  없는 말이 없다 — 있는 말을 사람처럼 안 쓴다(레지스터). 서린 정정 1·2(② 발명)와
  같은 경로(② → ⑥ 누출)는 재현됐다.
- 후보 (운영자 결정 대기, 규칙 추가 아님 — 어느 입력이 문체 권위인지의 변경):
  - `caption-writer-v2` — 예시 우선: 캐릭터의 예시 캡션(voice 예시 + 사람이 쓴
    recentPosts)의 길이·문장 꼴·비유 밀도를 따르고, premise는 **무슨 일이 있었나의
    기록이지 문장 재료가 아니다**. 형용사("시적")보다 예시가 이긴다.
  - `post-planner-v3` — "concretely enough…" 문장 삭제(서린 관측 1에서 이미 후보).
    한소이에선 발명이 없었으므로 이 문장 삭제만으로 1번 누출이 닫히지는 않는다
    (누출 자체는 ⑥의 읽기 방식).
  - 데이터: 이전 파이프라인이 만든 게시 캡션이 recentPosts에서 문체 근거로 작동
    하는 문제는 프롬프트 밖 — 게시 이력 정리는 운영자 판단.
- 판정: 보류(n=2, 캐릭터 2). 이미지 쪽 결함(관측 3 in `image-planner-v3`)이 ③에서
