...
**미정.** 조건당 2장이고 확산 모델은 비결정적이라 시드 운과 구분되지 않는다.
판정자도 평가 Agent가 아니라 사람이다. 다만 다섯 축이 **전부 같은 방향**으로
움직였고 모델 2종에서 재현됐다.

판정에 필요한 것: 조건당 4~6장, 그리고 생성 이미지 평가 Agent의 실제 점수.

### 코드 변경 후보 (미적용)

`falSupportsNegativePrompt(model)`이 false면 네거티브를 프롬프트에 이어붙이지
않는다. 지금은 부정을 지원하지 않는 모델에 부정을 텍스트로 밀어 넣고 있고,
그것이 이득인지 손해인지 한 번도 측정한 적이 없다. 표본을 채운 뒤 결정한다.
