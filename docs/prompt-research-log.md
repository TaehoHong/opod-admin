...
## post-planner-v3 — 구조화 페르소나의 역할과 출처 해석 (2026-09-17)

- 상태: 관찰 중 — 코드 반영, 실제 모델 비교 실험 전.
- 가설: v2 조각의 역할·주입 정책·출처를 잃고 원문과 최신 기억만 전달하면 제작자 메모와
  예시가 사실로 취급되고, 옛 `content_style`/`voice` 제목이 없는 캐릭터는 기획 자체가 막힌다.
- 변경: 입력 reader가 kind/injection/source와 Canon 연결을 보존하고 비공개·대화 시작 전용
  조각을 제외한다. 프롬프트는 동기·판단·긴장·관계를 작은 순간의 근거로 해석하고,
  매 게시물에서 드러낼 특성 체크리스트로 취급하지 않는다. example은 실제 사건이나 문장
  템플릿이 아니며, 출처 연결·날짜·누락된 validity를 현재의 확정 사실로 승격하지 않는다.
  출력 계약은 `post-plan-v2` 그대로다.
- 결과: 회귀 테스트에서 v2의 옛 제목 없는 기획, 실제 요청 본문의 비공개 조각 제외,
  후속 이미지 기획·캡션의 boundary 전달을 확인했다. 격리 DB E2E에서 오래된 관련 Canon
  검색과 캐릭터/삭제 격리를 확인했다. 실생성 사례나 자연스러움 점수 개선은 아직 측정하지 않았다.
- 판정: 입력 계약 수정은 유지. 문체와 순간 선택 품질에 관한 가설은 미검증이다.
- 다음: 같은 캐릭터의 작은 게시 순간을 대상으로 후보 채택·재기획·건너뛰기 기준을 별도 검토한다.
  현재 검색은 recall_keys 문자열 일치이며 의미 기반 검색은 이번 변경에 포함하지 않는다.
