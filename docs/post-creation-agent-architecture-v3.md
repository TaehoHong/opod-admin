...
| 11 | 생성 이미지가 사람 눈에 어색하다 (허리-골반 과장, 주름 없음, 접지 그림자 없음) | 운영자 가설 검증 — 제약 과다가 원인인가. 워커가 붙이는 네거티브 1,102자(전송본의 36%, 금지어 62개)를 제거한 어블레이션 | **다섯 축이 같은 방향으로 개선, 모델 2종에서 재현.** 판정은 표본 부족으로 보류 | research-log `negative-block-ablation` |
...
**금지어가 결함을 부른다(가설).** #11. 우리가 관측한 결함 다섯 개가 전부 금지
목록에 이름 그대로 있었고, 그 목록을 제거하자 결함이 줄었다. 부정을 지원하지
않는 모델에 부정을 긍정 텍스트로 밀어 넣고 있었다. 검증 중이지만, 사실이라면
"안전장치를 더 넣을수록 나빠진다"는 반직관적 결론이 된다.
