...
§18.8). #19에서 둘 더 — 기획 평가가 "누가 카메라를 드는가"를 안 보고
`capture_plausibility` 5를, 이미지 평가가 손·팔뚝만 계획된 컷에 등·머리가
나왔는데 `character_presentation` 5를 줬다. 공통 구조는 하나다: **검증자에게 "이게 괜찮은가?"를 묻고 답을 믿었다.
...
**규칙은 있는데 입력이 없다.** #19. 플래너 프롬프트에 "한 사람으로 동시에
가능해야 한다"가 있고 페르소나에 "뒤에서 따라 찍는 구도 금지"가 있는데도
3인칭 컷이 났다. 규칙이 약해서가 아니라 **그 규칙을 발동시킬 사실이 입력에서
빠졌기 때문**이다(`content_style`을 글쓰기 블록으로 분류해 ③에 안 보냄). 이런
결함을 프롬프트에 규칙을 더해 막으면 규칙 목록만 자라고(#8) 진짜 원인은 남는다.
순서를 고정한다: 결함이 나면 **먼저 그 Agent가 무엇을 받았는지**를 LLM 로그로
...
운영자 지시 "개선하기 전에, 이미지 구도가 반복적으로 무너지는 원인부터 찾아라."
개별 사건(#8·#15·#19)이 아니라 왜 **같은 축에서 계속** 나는지를 본다.
...
규칙과 섞여 있다. 계약(`characterVisualContext`)에 그 슬롯이 없으니 버전마다
전달 여부가 임의로 정해진다 — V2는 페르소나 전체를 넣고 규칙으로 읽게 했고,
V3 역할 분리에서 **입력(content_style 제외)과 규칙(촬영자 발명 금지) 둘 다
떨어졌다.** 주인 없는 데이터는 리팩터에서 유실되고, 유실을 잡는 회귀 장치가
없었다(문서에는 원칙이 남아 있었다 — 6.2 "촬영 습관은 persona에서 읽는다").

**원인 2 — captureSetup(기하)은 검증용이지 렌더링용이 아니다.** 파이프라인의
구도 검증(플래너 규칙, 기획 평가 `capture_plausibility`, ④ meta_leak 린트)은
전부 "카메라가 어디 있고 누가 드는가"라는 **기하의 정합성**을 텍스트로 본다.
그런데 모델에 닿는 것은 ④가 번역한 문장뿐이고, ④는 "composition·crop·capture
method를 추가하지 말라"는 범위에 묶여 있어 모델이 실제로 따르는 프레임 묘사
("한 손만 아래 가장자리에서 들어온다, 몸통은 없다")를 **아무도 쓰지 않는다.**
③의 `visibleParts: [양손, 팔뚝]`도 "얼굴 없음" 한 줄로만 번역됐다. 증거:
어블레이션에서 촬영 문구 수정 0/6, 장면 계약 수정 6/6; 15°/20°는 4/4 무시.
**모델은 기하를 읽지 않고 프레임 안의 내용을 읽는다.** 기하로 적고 기하로
검증하는 한, 검증을 다 통과한 계약도 구도를 정하지 못한다.

**원인 3 — 레퍼런스가 시점을 가져온다.** 집 장소의 환경 레퍼런스는 전부
"모서리 고정 와이드뷰"다(연남동 2/2, 현관 1/1). edit 모드 모델은 텍스트의
"와이드뷰를 복사하지 말라"보다 픽셀의 시점을 따른다(#12에서 이미 관측). 이번
어블레이션에서 레퍼런스를 빼면 1/3이 1인칭이 됐고(넣으면 0/9), 그러나 장면
계약을 바꾸면 레퍼런스가 있어도 6/6이었다. 지렛대 순서: **장면 계약 > 레퍼런스
시점 > 촬영 문구.** 정체성 레퍼런스가 거울 셀피인 서린은 그 시점을 물려받아
우연히 맞았고, 정체성 레퍼런스가 없는 손 컷(한소이)은 방 모서리 시점만 남았다.

**원인 4 — 이 축을 보는 눈이 없다.** 기획 평가 `capture_plausibility` 5, 이미지
평가 `capture_and_composition` 5·`character_presentation` 5. 15°→옆모습도 어떤
평가자도 지적하지 않았다. 재발이 지표에 안 잡히니 매번 운영자가 눈으로 발견하고
"새 결함"으로 들어와 규칙 한 줄이 추가된다.

**원인 5 — 그래서 고친 층이 늘 프롬프트 규칙 층이었다.** #8→#15→#19 세 번 다
사례는 잡히고 원칙은 일반화되지 않았다(§19.2 "사례-원칙"). 원인 1~4는 한 번도
건드리지 않았다. 구도 결함은 규칙이 모자라서가 아니라 **구도를 정하는 정보가
...
규칙이 아니라 **표현 불가능한 상태**가 된다. (b) 구도 어휘의 주인을 정한다 —
③ scene이 프레임 경계(무엇이 어느 가장자리에서 들어오고 무엇이 없는가)를 쓰거나,
④에게 visibleParts→프레임 묘사 번역을 허용한다. (c) 레퍼런스에 시점 태그(와이드/
POV)를 두고 1인칭 컷에는 와이드뷰를 보내지 않는 정책. (d) 구도 축 평가는 LLM
