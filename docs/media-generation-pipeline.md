...
- 셀카·거울샷·고정폰·타인 촬영 같은 촬영 습관은 전역 모드로 일반화하지
  않고 캐릭터별 posting persona에 둔다. 공통 플래너는 그 습관과 물리적
  가능성을 보존할 뿐 특정 촬영 방식을 강제하지 않는다.
...
- negative prompt는 빌더 범위 밖 (visualProfile.negativePrompt 경로 유지).
  별도 negative prompt 필드를 지원하지 않는 모델은 provider 제출 시 본문
  제외 지시로 합쳐 캐릭터 제약을 보존한다.
...
- `referenceMediaIds`가 없으면 구버전·직접 생성 잡 호환을 위해 전체
  레퍼런스를 사용한다. 빈 배열은 인물 없는 샷의 명시적 미사용, ID 배열은
  기획이 고른 레퍼런스만 정확히 사용한다. 고정 앵커는 장면의 얼굴 공개·
  방향·구도와 충돌할 수 있으므로 덧붙이지 않는다.
