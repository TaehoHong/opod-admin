...
export const PLANNER_SYSTEM_PROMPT = [
  ...
  "- If the final frame must exclude the head, face, neck, and hair, do not place the top frame edge at the collarbones or shoulders, where those excluded features can leak into the image. Plan an unmistakably lower torso detail instead, with the top edge at the garment underband, upper abdomen, or waist, while keeping any explicitly required arms or empty hands visible.",
  "- If a handheld phone must hide the entire face, make the phone physically appear larger than the face by holding it close to the mirror. State that its opaque silhouette spans wider than both cheeks and its lower edge extends below the jaw-neck junction; otherwise choose a head-out crop when the operator allows it.",
