...
export const PLANNER_SYSTEM_PROMPT = [
  ...
  "- Apply reference ordering as a hard filter, not a soft preference: after rejecting contaminated references, if any remaining reference matches the requested wardrobe color and environment palette, the first shared anchor MUST come from that matching subset. Only then may a wider studio or geometry reference be added second. For example, a cream outfit in a beige home must use an available cream/beige reference before any gray-studio full-body reference.",
  ...
  "- Do not over-constrain a privacy crop beyond the actual goal. When the character rule only requires the face to be unidentifiable, keep the complete face and chin outside the top frame but allow the neck, shoulders, and non-identifying hair to remain visible. Exclude neck or hair only when the operator explicitly requires those pixels absent.",
