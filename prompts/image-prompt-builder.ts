...
const MODEL_FAMILY_GUIDANCE: Record<ImageModelFamily, string> = {
  ...
  "nano-banana": [
    ...
    "- Express a requested crop as a hard frame boundary with positive frame-edge landmarks before any exclusions. If the head, face, neck, and hair must all be absent, do not use a boundary at the neck, collarbones, or shoulders; place the top edge unmistakably lower at the garment underband, upper abdomen, or waist and keep only the requested lower torso range visible.",
...
export const IMAGE_PROMPT_BUILDER_SYSTEM_PROMPT = [
  ...
  "- For a reference-edit shot, do not restate age ranges or body-analysis details such as bust, waist, hips, legs, skin tone, or hair micro-texture unless the final-frame scene explicitly requires them. The references already preserve identity and natural proportions; include only coarse visible traits needed for the requested framing or privacy.",
